package dev.lone64.NMSLib.nms.version.v1_21_R4.nbt;
...
public class NBTItemImpl_v1_21_R4 implements NBTItem {
    private final ItemStack item;
    private final ItemMeta meta;

    public NBTItemImpl_v1_21_R4(ItemStack item) {
        this.item = item;
    ...
    private <P, C> NBTItem setTag(String key, PersistentDataType<P, C> type, C value) {
        ...
        this.getPersistentDataContainer().set(Objects.requireNonNull(NamespacedKey.fromString("nms_tag_item:%s".formatted(key))), type, value);
    ...
    private <P, C> C getTag(String key, PersistentDataType<P, C> type) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        return getPersistentDataContainer().get(Objects.requireNonNull(NamespacedKey.fromString("nms_tag_item:%s".formatted(key))), type);
    }
    ...
    public NBTItem removeTag(String key) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        this.getPersistentDataContainer().remove(Objects.requireNonNull(NamespacedKey.fromString("nms_tag_item:%s".formatted(key))));
        this.getItem().setItemMeta(this.meta);
        return this;
    }
    ...
    public boolean hasTag(String key) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        return getPersistentDataContainer().has(Objects.requireNonNull(NamespacedKey.fromString("nms_tag_item:%s".formatted(key))));
    }
