package dev.lone64.NMSLib.nms.version.v1_20_R1.nbt;
...
public class NBTItemImpl_v1_20_R1 implements NBTItem {
    private final ItemStack item;
    private final ItemMeta meta;

    public NBTItemImpl_v1_20_R1(ItemStack item) {
        this.item = item;
    ...
    private <P, C> C getTag(String key, PersistentDataType<P, C> type) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        return this.getPersistentDataContainer().get(Objects.requireNonNull(NamespacedKey.fromString("nms_tag:%s".formatted(key))), type);
    }
    ...
    public boolean hasTag(String key) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        return this.getPersistentDataContainer().has(Objects.requireNonNull(NamespacedKey.fromString("nms_tag:%s".formatted(key))), PersistentDataType.BOOLEAN);
    }
