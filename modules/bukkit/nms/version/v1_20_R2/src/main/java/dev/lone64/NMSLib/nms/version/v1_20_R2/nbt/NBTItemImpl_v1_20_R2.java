package dev.lone64.NMSLib.nms.version.v1_20_R2.nbt;
...
public class NBTItemImpl_v1_20_R2 implements NBTItem {
    private final ItemStack item;
    private final ItemMeta meta;

    public NBTItemImpl_v1_20_R2(ItemStack item) {
        this.item = item;
    ...
    private <P, C> NBTItem setTag(String key, PersistentDataType<P, C> type, C value) {
        ...
        this.getPersistentDataContainer().set(NamespacedKey.fromString("nms_tag:%s".formatted(key)), type, value);
    ...
    private <P, C> C getTag(String key, PersistentDataType<P, C> type) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        return this.getPersistentDataContainer().get(NamespacedKey.fromString("nms_tag:%s".formatted(key)), type);
    }
    ...
    public NBTItem removeTag(String key) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        this.getPersistentDataContainer().remove(NamespacedKey.fromString("nms_tag:%s".formatted(key)));
        this.getItem().setItemMeta(this.meta);
        return this;
    }
    ...
    public boolean hasTag(String key) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        return this.getPersistentDataContainer().has(NamespacedKey.fromString("nms_tag:%s".formatted(key)), PersistentDataType.BOOLEAN);
    }
