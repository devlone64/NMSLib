package dev.lone64.NMSLib.nms.version.v1_20_R4.nbt;
...
@Getter
public class NBTEntityImpl_v1_20_R4 implements NBTEntity {
    private final Entity entity;

    public NBTEntityImpl_v1_20_R4(Entity entity) {
        this.entity = entity;
    }
    ...
    private <P, C> NBTEntity setTag(String key, PersistentDataType<P, C> type, C value) {
        ...
        this.getPersistentDataContainer().set(NamespacedKey.fromString("nms_tag_entity:%s".formatted(key)), type, value);
        return this;
    }

    private <P, C> C getTag(String key, PersistentDataType<P, C> type) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        return getPersistentDataContainer().get(NamespacedKey.fromString("nms_tag_entity:%s".formatted(key)), type);
    }
    ...
    public NBTEntity removeTag(String key) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        this.getPersistentDataContainer().remove(NamespacedKey.fromString("nms_tag_entity:%s".formatted(key)));
        return this;
    }
    ...
    public boolean hasTag(String key) {
        key = key.replace("-", "_").replace(" ", "_").toLowerCase();
        return getPersistentDataContainer().has(NamespacedKey.fromString("nms_tag_entity:%s".formatted(key)));
    }
}